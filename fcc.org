...
css: grid: {{c1::Define a grid's columns size and number}} with {{c2::grid-template-columns}}
...
css: grid: {{c1::Define a grid's rows size and number}} with {{c2::grid-template-rows}}
