...
**** Multiple Files Archives
***** For very large support and optimised compression: zip + zopfli
***** For large support and better compression: 7z
***** If large support is not required: zpaq
****** Deduplication
****** Incremental updates that only appends data at the end of the archive (7z modifies the file header)
**** Text Files: zstd
***** Large support
***** Very fast compression and even faster decompression
***** Perfect for text
** Project
*** Unit tests
*** Integration tests
...
*** Dev
**** TS
***** Set https://www.typescriptlang.org/tsconfig/#strict (Enabled by default with =deno=)
***** Set https://www.typescriptlang.org/tsconfig/#noImplicitOverride (Not enabled by default in =strict= mode)
***** Use https://typescript-eslint.io/rules/explicit-member-accessibility/ ?
** Computer/Server
*** Performance
**** Enable http/3 with the HTTPS DNS resource record
**** Enable noatime or at least relatime to improve filesystem performance
*** Privacy
**** Use dnscrypt-proxy :performance:

Set with at least two providers (for redundancy) with DNSSEC and DNS-over-HTTPS. With two servers
and caching, it should handle most cases of DNS servers failure. Make sure that
chrome/chromium/firefox use the system resolver (dnscrypt-proxy). If not, they may cache dns answers
themselves (on top of dnscrypt-proxy).
...
**** Bash vs Zsh
***** Redirect stdout to a file and stderr to the pipe

With Zsh, this will redirect stdout to the file and to the pipe because of
...
The following will also work, but you won't be able to pipe this without also duplicating stdout:
#+BEGIN_SRC shell
cmd 2> >(cmd) >/dev/null
#+END_SRC
...
Example:
#+BEGIN_SRC shell
(echo stdout; echo stderr >&2) 2>&1 >&- >/dev/null | rg -v stderr
#+END_SRC
...
:END:
*** DONE Building A Virtual Machine inside ChatGPT
:PROPERTIES:
:Source: https://web.archive.org/web/20221205120924/https://www.engraved.blog/building-a-virtual-machine-inside/
:Source+: https://news.ycombinator.com/item?id=33847479
:END:

Someone managed to make ChatGPT emulate a Linux environment. It looks it doesn't work anymore or
that it only works randomly.

*** DONE ChatGPT, rot13, and Daniel Kahneman
:PROPERTIES:
:Source: https://web.archive.org/web/20221218140359/https://jameswillia.ms/posts/chatgpt-rot13.html
:Source+: https://web.archive.org/web/20221219133322/https://news.ycombinator.com/item?id=33913845
:END:

ChatGPT apparently can't do rot13 reliably yet. It propably indicates a lack of mathematical
proficiency for now.
...
*** DONE Why Twitter Didn’t Go Down: From a Real Twitter SRE
:PROPERTIES:
:Source: https://web.archive.org/web/20221209203341/https://matthewtejo.substack.com/p/why-twitter-didnt-go-down-from-a
:Source+: https://web.archive.org/web/20221209203420/https://news.ycombinator.com/item?id=33701371
:END:
** Technology
